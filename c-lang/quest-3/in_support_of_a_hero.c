...
char *citynames[CITY_COUNT] = {
    "Esbjerg", "Helsingør", "Herning", "Horsens", "Kolding",
    "København", "Næstved", "Odense", "Randers", "Roskilde",
    "Silkeborg", "Vejle", "Viborg", "Aalborg", "Århus",
};
...
// mask is a bitmask of visited cities (2^15 = 32768 states)
int dp[1 << CITY_COUNT][CITY_COUNT];
int parent[1 << CITY_COUNT][CITY_COUNT]; // for path reconstruction

int main() {
    int FULL = (1 << CITY_COUNT) - 1;
    ...
    for (int mask = 1; mask <= FULL; mask++) {
        for (int u = 0; u < CITY_COUNT; u++) {
            ...
            for (int v = 0; v < CITY_COUNT; v++) {
                ...
                int newdist = dp[mask][u] + distances[u][v];
    ...
    // Find best last city before returning to 0
    int best = INF, last = -1;
    for (int u = 1; u < CITY_COUNT; u++) {
        if (dp[FULL][u] == INF) continue;
        int total = dp[FULL][u] + distances[u][0];
        if (total < best) {
            best = total;
            last = u;
        }
    ...
    path[CITY_COUNT] = 0; // return to start

    // Print result
    printf("Shortest route for Samantha: %d km\n\n", best);
    for (int i = 0; i <= CITY_COUNT; i++) {
        if (i < CITY_COUNT)
            printf("  %d. %s\n", i + 1, citynames[path[i]]);
        else
            printf("  → Back to %s\n", citynames[0]);
    }
